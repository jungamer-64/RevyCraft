use super::JE_26_1_DATA_VERSION;
use super::nbt::{
...
pub(super) fn write_level_dat(path: &Path, meta: &WorldMeta) -> Result<(), StorageError> {
    ...
    data.insert("DataVersion".to_string(), NbtTag::Int(JE_26_1_DATA_VERSION));
...
fn validate_data_version(data: &BTreeMap<String, NbtTag>) -> Result<(), StorageError> {
    ...
    if data_version != JE_26_1_DATA_VERSION {
    ...
    if version_id != JE_26_1_DATA_VERSION {
        return Err(StorageError::InvalidData(format!(
...
fn version_compound() -> NbtTag {
    ...
    version.insert("Id".to_string(), NbtTag::Int(JE_26_1_DATA_VERSION));
    version.insert("Name".to_string(), NbtTag::String(VERSION_NAME.to_string()));
