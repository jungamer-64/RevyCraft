...
pub(super) fn write_nbt(name: &str, tag: &NbtTag) -> Result<Vec<u8>, StorageError> {
    let mut raw = Vec::new();
...
pub(super) fn short_field(
    ...
    match compound.get(key) {
        ...
        Some(NbtTag::Int(value)) => i16::try_from(*value)
            .map_err(|_| StorageError::InvalidData(format!("field {key} did not fit into i16"))),
        _ => Err(StorageError::InvalidData(format!(
    ...
    match tag {
        ...
        NbtTag::Compound(values) => {
            for (name, value) in values {
                write_named_tag(writer, tag_type(value), name, value)?;
            }
            write_u8(writer, 0)?;
            Ok(())
        }
        ...
        NbtTag::LongArray(values) => {
            write_i32(
                writer,
                i32::try_from(values.len()).expect("long array length should fit into i32"),
            )?;
            for value in values {
...
fn read_tag_payload(reader: &mut impl Read, tag_type: u8) -> Result<NbtTag, StorageError> {
    match tag_type {
        ...
        7 => {
            ...
            if len < 0 {
                return Err(StorageError::InvalidData(
                    "byte array length was negative".to_string(),
                ));
            }
            let mut bytes =
                vec![0_u8; usize::try_from(len).expect("byte array length should fit into usize")];
        ...
        9 => {
            ...
            if len < 0 {
                return Err(StorageError::InvalidData(
                    "list length was negative".to_string(),
                ));
            }
            let mut values =
                Vec::with_capacity(usize::try_from(len).expect("list length should fit"));
            for _ in 0..len {
        ...
        10 => {
            ...
            loop {
                ...
                values.insert(
                    read_string_u16(reader)?,
                    read_tag_payload(reader, child_type)?,
                );
            }
        ...
        11 => {
            ...
            if len < 0 {
                return Err(StorageError::InvalidData(
                    "int array length was negative".to_string(),
                ));
            }
            let mut values =
                Vec::with_capacity(usize::try_from(len).expect("int array length should fit"));
            for _ in 0..len {
        ...
        12 => {
            ...
            if len < 0 {
                return Err(StorageError::InvalidData(
                    "long array length was negative".to_string(),
                ));
            }
            let mut values =
                Vec::with_capacity(usize::try_from(len).expect("long array length should fit"));
            for _ in 0..len {
...
fn read_string_u16(reader: &mut impl Read) -> Result<String, StorageError> {
    ...
    String::from_utf8(bytes)
        .map_err(|error| StorageError::InvalidData(format!("invalid utf-8 string: {error}")))
}
