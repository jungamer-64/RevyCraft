...
| `live.plugins` | allowlist、failure policy、watch flag | `reload_config` |
...
config source を再読込し、plugin selection と topology generation の両方を再評価します。allowlist、profile map、failure policy、admin principal の変更をまとめて反映したいときはこれを使います。
...
最新 config を読みますが、反映するのは `network` と `topology` だけです。plugin selection を変えたくないまま、listener / routing / default adapter / MOTD などを更新したいときに使います。
