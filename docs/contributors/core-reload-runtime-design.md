...
- `Core`
  selection / topology / transport を変えずに `ServerCore` だけを migration する
- `Full`
...
selection / topology を固定したまま core だけを差し替える mode です。

1. consistency write lock を取得
2. current selection と active topology generation を固定する
3. live runtime から `CoreRuntimeStateBlob` と `SessionReattachRecord` を export する
4. candidate core を materialize する
5. play session を candidate core へ reattach する
6. protocol / gameplay generation へ必要な resync event を発行する
7. すべて成功した場合のみ core owner を swap する
8. 途中で失敗した場合は旧 core を維持し、candidate を破棄する
