...
- `mc-core` は semantic な command / event / inventory state machine に徹し、raw slot layout や edition/version ごとの inventory quirks は protocol plugin 側で解釈します。
...
3. `crates/core/mc-core`
   protocol 非依存の semantic state machine です。world/player state、semantic inventory command の適用、event 生成を担います。
...
protocol plugin は packet codec だけでなく connection ごとの protocol-owned state も持てます。active window、JE の rejected transaction pending、Bedrock の container rewrite のような wire/version 依存 state は runtime ではなく protocol plugin が保持し、reload 時は export / import hook で引き継ぎます。
...
この境界では、core が受け取るのは semantic な `InventoryClickTarget::Slot(...)` と validation だけです。window-local raw slot、JE の clicked-item echo / reject-ack gating、Bedrock の authoritative inventory / active window rewrite は protocol plugin が `decode_play()` / `encode_play_event()` の時点で吸収します。runtime は protocol-specific な inventory bookkeeping を持ちません。

runtime は session ごとに `SessionCapabilitySet` と generation 情報を保持し、どの protocol / gameplay generation がその接続に見えているかを追跡します。

`GameplaySessionSnapshot` には actual protocol capability set と protocol/gameplay generation も入るため、version-sensitive な gameplay 分岐も plugin 側へ寄せられます。
...
現行の protocol ABI surface は `ProtocolPluginApiV2` で、exported symbol は `mc_plugin_protocol_api_v2` です。plugin ABI 全体は `3.5` を前提にしています。
...
6. inventory/version quirks を core や runtime に戻さず、protocol plugin の session-owned state と semantic command/event 境界を保つことがこの architecture の前提です。
