...
| crate | 役割 | 使いどころ |
| --- | --- | --- |
| `mc-plugin-api` | ABI `7.0`、manifest struct、host API、typed codec | host / runtime 実装、ABI 契約確認 |
...
ABI はすべて `CURRENT_PLUGIN_ABI`、すなわち `7.0` に揃います。通常の Rust plugin ではこれを手で上書きする必要はありません。
