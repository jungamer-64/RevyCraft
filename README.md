...
通常起動では、`cargo run -p xtask -- package-plugins` が `runtime/server.properties` を優先して読み、存在しない場合だけ `runtime/server.properties.example` に fallback します。`plugin-allowlist` に含まれる plugin だけを package し、workspace 外で持ち込んだ third-party plugin directory は消しません。
...
properties を明示したいときは次を使います。

```bash
cargo run -p xtask -- package-plugins --properties runtime/server.properties.example
```
