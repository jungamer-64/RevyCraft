...
- `xtask`: protocol plugin packaging task
...
phase 2 以降は protocol plugin が必須です。`server-bootstrap` は `dist/plugins/<plugin-id>/plugin.toml` を読み、そこから shared library を解決します。runtime は `target/` を直接読みません。
`plugin-reload-watch=true` か `RunningServer::reload_plugins()` を使うと、protocol plugin の generation swap を有効化できます。storage/auth/gameplay plugin は検出されても phase gate の都合で activate されません。
...
- `server-runtime` 自体は concrete protocol crate を直接組み込みません。`server-bootstrap` は built-in storage と plugin host だけを束ね、protocol は plugin から読み込みます。
