...
- `server-bootstrap`: built-in storage と plugin host を束ねる起動 binary
- `xtask`: protocol / gameplay plugin packaging task
...
phase 3 以降は protocol / gameplay plugin が必須です。`server-bootstrap` は `dist/plugins/<plugin-id>/plugin.toml` を読み、そこから shared library を解決します。runtime は `target/` を直接読みません。
`plugin-reload-watch=true` か `RunningServer::reload_plugins().await` を使うと、protocol plugin と gameplay plugin の generation swap を有効化できます。storage は引き続き built-in、auth plugin は検出されても activate されません。
...
`server-runtime` の integration test は shared test harness 経由で protocol / gameplay plugin を package/load します。
...
- `server-runtime` 自体は concrete protocol crate を直接組み込みません。`server-bootstrap` は built-in storage と plugin host だけを束ね、protocol / gameplay は plugin から読み込みます。
...
- plugin host config は `plugins-dir`, `plugin-allowlist`, `plugin-failure-policy`, `plugin-reload-watch`, `plugin-abi-min`, `plugin-abi-max`, `default-gameplay-profile`, `gameplay-profile-map` です。現状の failure policy は `quarantine` のみです。
