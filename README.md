...
Minecraft JE `1.7.10` / `1.8.x` / `1.12.2` と Bedrock `be-924` を同一プロセス・同一ポート番号で扱う Rust 製 server-only workspace です。runtime は protocol / gameplay / storage / auth / admin-ui plugin を packaged artifact から読み込み、`mc-plugin-host` がそれを `LoadedPluginSet` にまとめ、`ServerSupervisor` が listener / generation / session supervision を起動します。
...
- `[live.topology].enabled_adapters = ["je-5", "je-47", "je-340"]` で JE multi-version routing を有効化できます。
- `[live.topology].enabled_bedrock_adapters = ["be-924"]` で Bedrock baseline listener を有効化できます。
...
- Bedrock baseline `be-924 / protocol 924`
...
Java protocol integration tests は `mc-proto-test-support` crate を正規入口にし、`TestJavaProtocol` から protocol version / packet id / inventory slot codec 差分を導出します。`server-runtime` などの test code から `mc_proto_je_common::__version_support` を直接たどるのは unsupported path として扱います。
