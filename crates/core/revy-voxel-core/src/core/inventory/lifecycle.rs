...
pub(in crate::core) fn tick_active_container_state(
    ...
    if let Some(window) = state
        ...
        && window.world_position().is_some()
    {
        // Keep block-backed container state authoritative even when the local
        // tick does not emit a visible window diff for the active viewer.
