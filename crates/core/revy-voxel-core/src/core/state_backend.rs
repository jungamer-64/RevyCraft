...
use revy_core::overlay::{apply_optional_entries, apply_optional_entry};
...
impl TxOverlay {
    ...
    pub(super) fn materialize_into(
        ...
        apply_optional_entries(&mut base.world.block_entities, self.block_entities);
        apply_optional_entries(&mut base.world.container_viewers, self.container_viewers);
        apply_optional_entries(&mut base.world.saved_players, self.saved_players);
        apply_optional_entries(
            &mut base.sessions.player_sessions,
            self.player_sessions
                .into_iter()
                .filter(|(player_id, _)| !skipped_players.contains(player_id)),
        );
        apply_optional_entries(
            &mut base.entities.players_by_player_id,
            self.players_by_player_id
                .into_iter()
                .filter(|(player_id, _)| !skipped_players.contains(player_id)),
        );
        apply_optional_entries(
            &mut base.entities.entity_kinds,
            self.entity_kinds
                .into_iter()
                .filter(|(entity_id, _)| !skipped_entities.contains(entity_id)),
        );
        apply_optional_entries(
            &mut base.entities.player_identity,
            self.player_identity
                .into_iter()
                .filter(|(entity_id, _)| !skipped_entities.contains(entity_id)),
        );
        apply_optional_entries(
            &mut base.entities.player_transform,
            self.player_transform
                .into_iter()
                .filter(|(entity_id, _)| !skipped_entities.contains(entity_id)),
        );
        apply_optional_entries(
            &mut base.entities.player_vitals,
            self.player_vitals
                .into_iter()
                .filter(|(entity_id, _)| !skipped_entities.contains(entity_id)),
        );
        apply_optional_entries(
            &mut base.entities.player_inventory,
