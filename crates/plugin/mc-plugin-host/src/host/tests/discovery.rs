...
fn discover_rejects_duplicate_plugin_ids() -> Result<(), RuntimeError> {
    ...
    let error = match plugin_host_from_config(&bootstrap_config_with_plugins_dir(
...
fn all_protocol_plugins_register_and_resolve() {
    ...
    let je_handshake = je_handshake_frame(404);
    ...
    assert_eq!(je_intent.protocol_number, 404);
