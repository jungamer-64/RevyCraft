...
fn remote_admin_principal(
    path: PathBuf,
    token: &str,
    permissions: Vec<crate::runtime::AdminPermission>,
...
-> Result<(), RuntimeError> {
    ...
    let alpha_token_path = temp_dir.path().join("admin").join("alpha.token");
    let beta_token_path = temp_dir.path().join("admin").join("beta.token");
    write_remote_admin_token(&alpha_token_path, "alpha-token")?;
    ...
    initial.admin.grpc.principals.insert(
        ...
        remote_admin_principal(
            alpha_token_path.clone(),
            "alpha-token",
            vec![crate::runtime::AdminPermission::Status],
        ),
    );
    write_server_toml(&config_path, &initial)?;
